...
import { Key, AlertCircle, KeyRound } from "lucide-react"
import { loginJwt } from "@/lib/auth-actions"
...
export default function JwtLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await loginJwt(username, password)
      if (result.success) {
        router.push("/dashboard-jwt")
      } else {
        ...
        <div className="w-full max-w-7xl mx-auto grid items-start gap-6 px-3 sm:px-4 lg:px-6 grid-cols-1 lg:grid-cols-2 lg:gap-10">
          <div className="space-y-3 order-2 lg:order-1 w-full">
            ...
            <div className="space-y-4 rounded-lg border bg-gray-50 p-3 sm:p-4 w-full overflow-hidden">
              <h3 className="font-semibold text-sm sm:text-base break-words">JWT認証の特徴:</h3>
              <ul className="list-disc pl-5 space-y-1 text-xs sm:text-sm break-words">
                <li>サーバー側でセッション状態を保持しない（ステートレス）</li>
                <li>トークンに必要な情報を含める（自己完結型）</li>
                <li>署名によりトークンの改ざんを検出可能</li>
                <li>スケーリングが容易（サーバー間で状態共有が不要）</li>
          ...
          <div className="w-full max-w-md mx-auto space-y-4 order-1 lg:order-2">
            <Card className="w-full overflow-hidden">
              <CardHeader>
                <CardTitle className="text-lg sm:text-xl break-words">JWT認証でログイン</CardTitle>
            ...
            <div className="text-center text-xs sm:text-sm">
              <Link href="/login-traditional" className="underline break-words">
                従来のセッション認証を試してみる
              </Link>
            </div>
          </div>
